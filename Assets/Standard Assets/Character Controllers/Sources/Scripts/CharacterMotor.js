...
var movement : CharacterMotorMovement = CharacterMotorMovement();
	var carac_speed: float;
	var control_vitesse: float;
    var maxFSpeed: float;
    var maxSSpeed: float;
    var maxBSpeed: float;
...
class CharacterMotorSliding {
	...
	//Implémentation de la caractéristique de vitesse
	

	var sidewaysControl : float = 15;
...
carac_speed = recupc.carac_vitesse;
	

 control_vitesse = (10 + carac_speed) / sliding.sidewaysControl;


 maxFSpeed = movement.maxForwardSpeed * control_vitesse;
 maxSSpeed = movement.maxSidewaysSpeed * control_vitesse;
 maxBSpeed = movement.maxBackwardsSpeed * control_vitesse;
...
private function ApplyInputVelocityChange (velocity : Vector3) {	
	...
	if (grounded && TooSteep()) {
		...
		desiredVelocity = desiredVelocity + projectedMoveDir * sliding.speedControl + (inputMoveDirection - projectedMoveDir) * control_vitesse;
...
function MaxSpeedInDirection (desiredMovementDirection : Vector3) : float {
	if (desiredMovementDirection == Vector3.zero)
		...
		var zAxisEllipseMultiplier : float = (desiredMovementDirection.z > 0 ? maxFSpeed : maxBSpeed) / maxSSpeed;
		var temp : Vector3 = new Vector3(desiredMovementDirection.x, 0, desiredMovementDirection.z / zAxisEllipseMultiplier).normalized;
		var length : float = new Vector3(temp.x, 0, temp.z * zAxisEllipseMultiplier).magnitude * maxSSpeed;
		return length;
	}
}
